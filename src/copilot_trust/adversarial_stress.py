...
RULE_TARGETS = {
    "identity_dispersion_multisignal": ["credential_sharing"],
    "shared_token_multisignal": ["token_misuse"],
    "scripted_usage_multisignal": ["scripted_automation"],
    "quota_evasion_multisignal": ["quota_evasion"],
    "policy_signal_escalation": ["policy_abuse"],
}
...
def _metrics(df: pd.DataFrame, pred: pd.Series) -> dict[str, float]:
    y = df.label.astype(int); p = pred.astype(bool)
...
def _target_recall(df: pd.DataFrame, pred: pd.Series, rule_name: str) -> tuple[int, float]:
    target_types = RULE_TARGETS[rule_name]
    target = df.ground_truth_abuse_type.isin(target_types)
    n = int(target.sum())
    if n == 0:
        return 0, float("nan")
    return n, float(pred.astype(bool).loc[target].mean())


def _apply_adaptation(df: pd.DataFrame, strategy: str, strength: float) -> pd.DataFrame:
    out = df.copy(); abuse = out.label.eq(1); strength = float(np.clip(strength, 0.0, 1.0))
...
def evaluate_adversarial_adaptation(scores: pd.DataFrame, out_dir: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    Two recall views are retained:
    - overall known-abuse recall, useful for portfolio-level coverage;
    - target-scenario recall, useful for measuring brittleness of the abuse family a rule was designed to detect.

    This is an analytical resilience test, not a recipe for bypassing a real product control.
    """
    out_dir = Path(out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    holdout = scores.loc[scores.split.eq("holdout")].copy(); development = scores.loc[scores.split.eq("train")].copy()
    velocity_cut = float(development.requests_per_active_day.quantile(0.75)); quota_cut = float(development.quota_pressure.quantile(0.75))
    strategies = ["velocity_smoothing","identity_fragmentation","token_rotation","quota_spreading","policy_signal_suppression","multi_signal_blending"]
    ...
    baseline_target = {name: _target_recall(holdout, pred, name) for name, pred in baseline_preds.items()}

    rows: list[dict] = []
    for strategy in strategies:
        for strength in strengths:
            adapted = _apply_adaptation(holdout, strategy, strength); preds = _rule_predictions(adapted, velocity_cut, quota_cut)
            for rule_name, pred in preds.items():
                m = _metrics(adapted, pred); base = baseline_metrics[rule_name]
                target_n, adapted_target_recall = _target_recall(adapted, pred, rule_name); _, baseline_target_recall = baseline_target[rule_name]
                target_drop = baseline_target_recall - adapted_target_recall if target_n else float("nan")
                rows.append({
                    "strategy": strategy,
                    "adaptation_strength": strength,
                    "rule_name": rule_name,
                    "target_abuse_types": "|".join(RULE_TARGETS[rule_name]),
                    "target_accounts_holdout": target_n,
                    "baseline_target_recall": baseline_target_recall,
                    "adapted_target_recall": adapted_target_recall,
                    "target_recall_drop": target_drop,
                    ...
                    "evaluation_note": "target-scenario recall is benchmark-only and uses synthetic abuse labels only after rule predictions are fixed",
                    "simulation_boundary": "coarse synthetic stress test; not a real-product bypass procedure",
                })
    stress = pd.DataFrame(rows); stress.to_csv(out_dir / "adversarial_rule_stress.csv", index=False)

    defense_rows: list[dict] = []
    base_union = pd.concat(baseline_preds, axis=1).any(axis=1); base_union_metrics = _metrics(holdout, base_union)
    for strategy in strategies:
        for strength in strengths:
            adapted = _apply_adaptation(holdout, strategy, strength); preds = _rule_predictions(adapted, velocity_cut, quota_cut)
            union = pd.concat(preds, axis=1).any(axis=1); m = _metrics(adapted, union)
            defense_rows.append({
                "strategy": strategy, "adaptation_strength": strength, "defense": "multi_rule_union_diagnostic",
                "baseline_recall": base_union_metrics["recall"], "adapted_recall": m["recall"], "recall_drop": base_union_metrics["recall"] - m["recall"],
                "precision": m["precision"], "false_positive_rate": m["false_positive_rate"], "review_workload_share": m["review_workload_share"],
                "policy_boundary": "diagnostic only; requires human-review capacity and user-impact guardrails",
            })
    defense = pd.DataFrame(defense_rows); defense.to_csv(out_dir / "defense_in_depth_stress.csv", index=False)

    comparable = stress[stress.target_accounts_holdout > 0].copy()
    worst_single = comparable.sort_values("target_recall_drop", ascending=False).iloc[0] if len(comparable) else stress.sort_values("recall_drop", ascending=False).iloc[0]
    worst_defense = defense.sort_values("recall_drop", ascending=False).iloc[0]
    summary = {
        "worst_single_rule": {
            "rule_name": str(worst_single.rule_name), "target_abuse_types": str(worst_single.target_abuse_types), "target_accounts_holdout": int(worst_single.target_accounts_holdout),
            "strategy": str(worst_single.strategy), "adaptation_strength": float(worst_single.adaptation_strength),
            "baseline_target_recall": None if pd.isna(worst_single.baseline_target_recall) else float(worst_single.baseline_target_recall),
            "adapted_target_recall": None if pd.isna(worst_single.adapted_target_recall) else float(worst_single.adapted_target_recall),
            "target_recall_drop": None if pd.isna(worst_single.target_recall_drop) else float(worst_single.target_recall_drop),
            "baseline_overall_recall": float(worst_single.baseline_recall), "adapted_overall_recall": float(worst_single.adapted_recall), "overall_recall_drop": float(worst_single.recall_drop),
        },
        "worst_defense_in_depth": {
            "strategy": str(worst_defense.strategy), "adaptation_strength": float(worst_defense.adaptation_strength),
            "baseline_recall": float(worst_defense.baseline_recall), "adapted_recall": float(worst_defense.adapted_recall), "recall_drop": float(worst_defense.recall_drop),
        },
    ...
    (out_dir / "adversarial_stress_summary.json").write_text(json.dumps(summary, indent=2)); return stress, defense, summary


def build_evasion_regression_gates(stress: pd.DataFrame, defense: pd.DataFrame, out_dir: str | Path) -> pd.DataFrame:
    comparable = stress[stress.target_accounts_holdout > 0].copy() if "target_accounts_holdout" in stress else stress
    max_single_drop = float(comparable.target_recall_drop.max()) if len(comparable) and comparable.target_recall_drop.notna().any() else float(stress.recall_drop.max()) if len(stress) else 0.0
    ...
    rows = [
        {"gate":"single_rule_brittleness_visible","observed":max_single_drop,"guardrail":"must be measured, not hidden","status":"pass","action":"record largest target-scenario recall degradation and identify brittle rule family"},
    ...
    gates = pd.DataFrame(rows); gates.to_csv(Path(out_dir) / "evasion_regression_gates.csv", index=False); return gates
