...
def build_dashboard(scores,metrics,slices,coverage,mitigation,mitigation_daily,daily_monitor,alerts,dq,rules,review_feedback,action_register,review_capacity,drift,calibration,queue_sla,queue_capacity,rule_registry,out_path: str|Path)->None:
    ...
    if len(billing):
        entitlement_fig=px.scatter(billing,x="family_accounts",y="family_usage_ratio",size="near_limit_accounts",color="shared_billing_context",hover_name="billing_family_ref",hover_data=["cycle_index","candidate_multi_account_evasion","reason_codes"],title="Billing-family entitlement pressure")
    else: entitlement_fig=go.Figure()
    ...
    severe_alerts=int(alerts.severity.isin(["high","critical"]).sum()) if len(alerts) and "severity" in alerts else 0; breached=int(queue_sla.sla_breached.sum()) if len(queue_sla) and "sla_breached" in queue_sla else 0
    evidence_ready=int(power.evidence_sufficient_for_policy_review.sum()) if len(power) else 0; one_fte=queue_sim.iloc[(queue_sim.analyst_fte-1.0).abs().argmin()] if len(queue_sim) else None
    ...
    cards=[("Holdout AP",f"{metrics['average_precision']:.3f}"),("Top-10% review precision",_pct(top10.precision_at_capacity) if top10 is not None else "n/a"),("Top-10% known-abuse recall",_pct(top10.known_abuse_recall_at_capacity) if top10 is not None else "n/a"),("Worst reportable FPR",_pct(worst_fpr)),("High / critical alerts",str(severe_alerts)),("Evidence-ready rules",str(evidence_ready)),("Candidate billing families",str(candidate_families)),("1-FTE final replay backlog",str(int(one_fte.final_backlog)) if one_fte is not None else "n/a"),("Synthetic SLA breaches",str(breached))]
    ...
    </main></body></html>"""; out_path.write_text(page,encoding="utf-8")
...
Synthetic cycle-level analysis identifies **{candidate_families}** billing-family cycles for multi-account entitlement-pressure review. This is a lead only: managed enterprise billing, shared organizational payment context, plan changes, credits, and legitimate multi-seat usage must be checked before escalation.
...
- Use scores, billing-family signals, and candidate rules to prioritize human investigation.
