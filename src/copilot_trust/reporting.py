...
def build_dashboard(
    ...
    mitigation_daily: pd.DataFrame,
    daily_monitor: pd.DataFrame,
    alerts: pd.DataFrame,
    dq: pd.DataFrame,
    rules: pd.DataFrame,
    review_feedback: pd.DataFrame,
    action_register: pd.DataFrame,
    review_capacity: pd.DataFrame,
    drift: pd.DataFrame,
    calibration: pd.DataFrame,
    queue_sla: pd.DataFrame,
    queue_capacity: pd.DataFrame,
    rule_registry: pd.DataFrame,
    out_path: str | Path,
) -> None:
    """Build a self-contained decision-center page from aggregate benchmark artifacts."""
    ...
    map_fig = px.scatter(
        holdout,
        x="requests_per_active_day",
        y="unique_devices",
        size="unique_ips",
        color="risk_score",
        hover_name="account_id",
        hover_data=["plan", "managed_infrastructure", "reason_codes"],
        title="Investigation map: velocity × device dispersion",
    )

    reportable = slices[slices.reportable.eq(1)].copy() if "reportable" in slices.columns else slices.copy()
    fpr_fig = px.bar(
        reportable.sort_values("false_positive_rate", ascending=False).head(12),
        x="slice_value",
        y="false_positive_rate",
        color="slice_type",
        title="False-positive rate by reportable operational slice",
    )
    ...
    cap_fig = px.line(
        review_capacity,
        x="review_capacity_share",
        y=["precision_at_capacity", "known_abuse_recall_at_capacity"],
        markers=True,
        title="Review-capacity trade-off",
    )
    ...
    cal_fig = go.Figure()
    cal_fig.add_trace(go.Scatter(x=calibration.mean_predicted_risk, y=calibration.observed_known_abuse_rate, mode="markers+lines", name="Observed"))
    cal_fig.add_trace(go.Scatter(x=[0, 1], y=[0, 1], mode="lines", name="Perfect calibration"))
    cal_fig.update_layout(title="Holdout risk calibration diagnostic", xaxis_title="Mean predicted risk", yaxis_title="Observed known-abuse rate")
    ...
    cards = [
        ...
        ("Top-10% known-abuse recall", _pct(top10.known_abuse_recall_at_capacity) if top10 is not None else "n/a"),
        ("Worst reportable FPR", _pct(worst_fpr)),
        ("High / critical alerts", str(severe_alerts)),
    ...
    <h1>AI Abuse Analytics Decision Center</h1>
    <p class='sub'>Privacy-safe synthetic benchmark for AI developer-tool Trust & Safety analytics. Scores and rules prioritize investigation; they do not authorize automatic enforcement.</p>
    ...
    <h2>Operations</h2><div class='grid'><div class='panel'>{_fig_html(queue_fig)}</div><div class='panel'><h3>Queue SLA snapshot</h3>{_table_html(queue_sla[['account_id','risk_score','priority_tier','queue_age_hours','sla_hours','sla_breached']].head(15) if len(queue_sla) else queue_sla,15)}</div></div>
...
def build_executive_brief(
    ...
    reportable = slices[slices.reportable.eq(1)] if "reportable" in slices.columns else slices
    ...
    cap = review_capacity.iloc[(review_capacity.review_capacity_share - 0.10).abs().argmin()] if len(review_capacity) else None
    mit = mitigation.iloc[0] if len(mitigation) else None
...
- Require sufficient holdout volume before rule promotion.
