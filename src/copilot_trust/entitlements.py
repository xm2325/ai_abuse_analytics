...
def build_synthetic_entitlement_ledger(data_dir: str | Path, cycle_days: int = 28) -> pd.DataFrame:
    ...
    # Independent synthetic entitlement contract. Normal plan usage is below the review threshold,
    # while quota-evasion and power-user patterns can approach or exceed it for different reasons.
    ...
    # Only a bounded subset shares organization billing so the benchmark contains a clear legitimate confounder.
    ...
    while cycle_start <= last_day:
        ...
        for _, r in accounts.iterrows():
            ...
            if account_id in quota_family:
                billing_family = quota_family[account_id]
                billing_context = "synthetic_linked_billing_for_quota_evasion"
            elif account_id in managed_family:
                billing_family = managed_family[account_id]
            ...
            rows.append({
                ...
                "entitlement_limit_per_active_day": plan_daily_units[str(r.plan)],
                "entitlement_contract_version": "synthetic-plan-contract-v1",
...
def evaluate_entitlement_abuse(data_dir: str | Path, out_dir: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for _, e in ledger.iterrows():
        ...
        usage_rows.append({
            ...
            "near_limit": int(usage_ratio >= 0.90),
        })
    ...
    family = usage.groupby(["cycle_index", "cycle_start", "cycle_end", "billing_family_ref"], as_index=False).agg(
        ...
        synthetic_linked_quota_accounts=("billing_context", lambda s: int((s == "synthetic_linked_billing_for_quota_evasion").sum())),
    ...
    family["reason_codes"] = np.where(
        family.candidate_multi_account_evasion.eq(1),
        "multi_account_same_billing_family|simultaneous_entitlement_pressure",
        np.where(family.shared_billing_context.eq(1), "shared_billing_requires_enterprise_context_review", "no_multi_account_pressure"),
    )
